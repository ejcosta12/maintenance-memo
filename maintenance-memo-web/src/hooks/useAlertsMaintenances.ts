...
function useAlertsMaintenances (maintenancesRequest: MaintenanceMotor[]) {
    ...
    if (maintenancesRequest.length > 0) {
      const maintenancesPlusAlerts = maintenancesRequest.map((maintenance) => {
        if (maintenance.valueIP < 2) {
            ...
            } else if (maintenance.valueIP > 1) {
                const { colorAlert } = polarizationIndex[1];
                return ({
                  ...maintenance,
                  colorAlert,
                });
              } else if (maintenance.valueIP === 0) {
          ...
          } else if (maintenance.valueIA < 1.25) {
            ...
            } else if (maintenance.valueIA > 1) {
              const { colorAlert } = absorptionIndex[1];
                return ({
                  ...maintenance,
                  colorAlert,
                });
              } else if (maintenance.valueIA === 0) {
          ...
          } else if (differenceInMonths(new Date(), new Date(maintenance.created_at)) > 6) {
