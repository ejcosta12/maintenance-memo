...
function useAlertsMotors (motorsRequest: Motor[]) {
    ...
    if (motorsRequest.length > 0) {
      const motorsPlusAlerts = motorsRequest.map((motor) => {
        if (motor.lastValueIP < 2) {
          ...
          } else if (motor.lastValueIA < 1.25) {
            setCount((value) => value + 1);
          ...
          } else if (differenceInMonths(new Date(), new Date(motor.dateLastMaintenance)) > 6) {
              setCount((value) => value + 1);
