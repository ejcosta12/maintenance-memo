...
const UpdateLocationMotor: React.FC = () => {
  ...
  useEffect(() => {
    async function loadMotor(): Promise<void> {
      ...
      if (!!responseMotor) {
        const motorData = motorsPlusAlerts.find((motor) => {
          return (
            motor.uuIdMotor === responseMotor.uuId
          );
        });
        motorData ? setMotor({
          ...responseMotor,
    ...
    >
      ...
      <AreaSection>
        ...
        <div className='motor-section'>
          <div>
            <span>Número do motor: <strong>{ motor.numId }</strong></span>
            <span>
              Tipo:
                <strong>{
            ...
            <span>Potência: <strong>{ motor.power } CV</strong></span>
          </div>
          <div>
            <span>Unidade: <strong>{nameLocalUnit}</strong></span>
